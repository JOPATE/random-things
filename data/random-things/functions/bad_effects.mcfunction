...
execute as @e[tag=!good_effects,distance=..15] run effect give @s wither 7 2 false
execute as @e[tag=!good_effects,distance=..15] run tag @s add not_good_effects
particle flash ~ ~1 ~
playsound ambient.cave master @a[distance=..35] ~ ~ ~ 
scoreboard players set @s bad_effects_cooldown 600
clear @s[nbt=!{ playerGameType:1}] emerald{CustomModelData:402013} 1
